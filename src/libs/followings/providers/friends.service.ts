...
export class FriendsService {
  ...
  public getFollowingsOtherUser(
    ...
    const followings$ = from(this.usersService.queryMe(userId)).pipe(
      map((existsUser) => {
        if (!existsUser) {
          throw new BadRequestException(`Can't not find user ${userId}`);
        }
